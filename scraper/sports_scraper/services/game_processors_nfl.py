...
def process_game_pbp_nfl(session, game, *, client=None, live_poll: bool = False) -> GameProcessResult:
    ...
    if payload.plays:
        if live_poll and live_pbp_payload_unchanged("NFL", game.id, payload.plays):
            return result

        upsert_kwargs = {"source": "espn_nfl_api"}
        if live_poll:
            upsert_kwargs["create_snapshot"] = should_create_live_pbp_snapshot("NFL", game.id)
